...
Generated: 2026-04-21 11:01:08.742956
...
import { handleNewMember, handleMemberLeave, showRank, showTopInvites, showMyInvites, resetUserInvites, loadGroupData, saveGroupData, getUserName } from './plugins/inviteRank.js';
...
if (command === '!addpoints') {
    ...
    if (!isAdminUser) {
        await this.sock.sendMessage(groupJid, {
            text: '❌ Apenas administradores podem adicionar pontos!'
        }, { quoted: message });
        return;
    }
    ...
    if (!target || !points) {
        await this.sock.sendMessage(groupJid, {
            text: '❌ Use: !addpoints @usuario 50\nou\n!addpoints 5511999999999 50'
        }, { quoted: message });
        return;
    }
    ...
    if (isNaN(points) || points <= 0) {
        await this.sock.sendMessage(groupJid, {
            text: '❌ Pontos inválidos! Use um número positivo.'
        }, { quoted: message });
        return;
    }
    
    // CONVERTER PARA ID DO WHATSAPP
    let userId = target;
    
    // Se for número (só dígitos), adicionar @s.whatsapp.net
    if (/^\d+$/.test(target)) {
        userId = `${target}@s.whatsapp.net`;
    }
    // Se for menção, extrair o ID
    ...
    console.log(`🔍 Adicionando pontos para: ${userId}`);
    
    // Buscar o nome do usuário
    const userName = await getUserName(this.sock, groupJid, userId);
    ...
    await this.sock.sendMessage(groupJid, {
        text: `✅ Adicionado *${points}* pontos para *${userName}*!\n\n📊 Agora tem *${data.users[userId].points}* pontos.`
    }, { quoted: message });
    return;
}
            // ========================================
...
// Buscar nome do usuário no grupo
async function getUserName(sock, groupJid, userId) {
    try {
        const metadata = await sock.groupMetadata(groupJid);
        const participant = metadata.participants.find(p => p.id === userId);
...
export async function handleNewMember(sock, groupJid, newMemberId, inviterId, inviterName) {
    try {
        console.log(`🎯 Processando convite: ${inviterId} convidou ${newMemberId}`);
        
        // Buscar nome do convidador se não veio
        ...
        if (!alreadyInvited) {
            ...
            await sock.sendMessage(groupJid, { text: message });
            console.log(`✅ Pontos registrados: ${inviterDisplayName} +${pointsEarned}`);
        } else {
            console.log(`⚠️ ${inviterDisplayName} já havia convidado esta pessoa`);
        }
...
export async function showMyInvites(sock, message, groupJid, senderId) {
    try {
        ...
        if (!data.users[senderId] || data.users[senderId].invites.length === 0) {
            await sock.sendMessage(groupJid, {
                text: '📊 Você ainda não convidou ninguém!\n\nUse o botão "Adicionar" para convidar amigos e ganhar pontos! 🎉'
            }, { quoted: message });
            return;
        }
        ...
        // Mostrar últimos 10 convites (buscar nomes)
        const lastInvites = user.invites.slice(-10);
        for (let i = 0; i < lastInvites.length; i++) {
            const invitedId = lastInvites[i];
...
// plugins/inviteRank.js - SUBSTITUIR esta função
...
async function getUserName(sock, groupJid, userId) {
    try {
        ...
        const participant = metadata.participants.find(p => {
            const pId = p.id.includes('@') ? p.id.split('@')[0] : p.id;
            return pId === cleanId || p.id === userId;
        });
        ...
        if (participant) {
            ...
            if (participant.pushname && participant.pushname !== 'null') {
                return participant.pushname;
            }
            if (participant.id) {
                const number = participant.id.split('@')[0];
                if (number.length >= 12) {
                    // Mostra os últimos 4 dígitos se for número
...
export { loadGroupData, saveGroupData, getUserName };
